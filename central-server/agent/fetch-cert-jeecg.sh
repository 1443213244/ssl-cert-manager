#!/bin/bash
# JeeCG-Boot / Nginx 服务器 - SSL证书自动拉取Agent
# 定时从中央服务器拉取证书，重载Nginx
...
# ============ 函数定义 ============
log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [JEECG] $1" >> "${LOG_FILE}"
}
...
if systemctl is-active --quiet nginx 2>/dev/null; then
    ...
    log "Nginx is running but not managed by systemd, sending HUP signal..."
    pkill -HUP nginx && log "Sent SIGHUP to Nginx" || log "WARN: Failed to reload Nginx"
fi

# 重载 Apache/httpd（如果运行中）
if systemctl is-active --quiet httpd 2>/dev/null; then
    systemctl reload httpd && log "httpd reloaded"
